...
const CardContainer = ({ post, id }: { post: PostType; id: string }) => {
  ...
  return (
    <div className="w-full flex justify-center max-md:flex-col ">
      ...
      />
      <div
        className={`w-full h-20 fixed bottom-0 flex  ${
          isClicked ? "" : "blur-3xl"
        } `}
      >
        <div className="w-1/2 mt-5 flex items-center justify-center h-full bg-blue-700">
          <h3 className="text-6xl font-extrabold stroke-text">
            {Math.round((post.options[0].points.length / totalPoints) * 100)}%{" "}
          </h3>
        </div>
        <div className="w-1/2 mt-5 flex items-center justify-center h-full bg-red-700">
          <h3 className="text-6xl font-extrabold stroke-text">
